...
class CrossModalAttention(nn.Module):
    ...
    def forward(self, query, key, value):
        attn_output, _ = self.attention(
            query.permute(1, 0, 2),  # [Seq, Batch, Dim]
            key.permute(1, 0, 2),
            value.permute(1, 0, 2)
        )
        return self.norm(attn_output.permute(1, 0, 2))  # [Batch, Seq, Dim]


class FusionBlock(nn.Module):
    def __init__(self, in_dim=1024 * 2, out_dim=1024):
        super().__init__()
    ...
    def forward(self, visual_feat, text_feat):
        ...
        return gate * self.proj(combined)


def roi_pooling_with_relative_coordinates(feature_map, rois, output_size):
    """
    ...
    # Convert from Cx, Cy, W, H (ratios) to x_min, y_min, x_max, y_max (in pixels)
    roi_start_w = (rois[:, 1] - rois[:, 3] / 2) * feature_map.shape[3]
    roi_start_h = (rois[:, 2] - rois[:, 4] / 2) * feature_map.shape[2]
    roi_end_w = (rois[:, 1] + rois[:, 3] / 2) * feature_map.shape[3]
    roi_end_h = (rois[:, 2] + rois[:, 4] / 2) * feature_map.shape[2]

    # Prepare rois in the format [batch_index, x_min, y_min, x_max, y_max]
    rois_pixel_coordinates = torch.stack([rois[:, 0], roi_start_w, roi_start_h, roi_end_w, roi_end_h], dim=1)
    # print(rois_pixel_coordinates)
    new = rois_pixel_coordinates[:, 1:]

    # Using roi_align
    output_align = roi_align(feature_map, [new], output_size=output_size)
...
class XVLMBase(nn.Module):
    def __init__(self, config=None, load_vision_params=False, load_text_params=False,
        ...
        self.crossmodal_attention = CrossModalAttention(embed_dim=768)
        self.fusion_block = FusionBlock(in_dim=768 * 2, out_dim=768)
        ...
        # 回归器预测bbox
        self.bbox_regressor1 = nn.Sequential(
            nn.Linear(1024, 64),
            nn.ReLU(),
            nn.Linear(64, 4)  # 假设bbox由x1,y1,x2,y2组成
        )
        # 回归器预测bbox
        ...
        self.roi_align = RoIAlign(output_size=(12, 12), spatial_scale=1.0, sampling_ratio=2.0)
        self.fusion_proj = nn.Sequential(
            nn.Linear(1536, 1024),
            nn.LayerNorm(1024)
        )
        # self.roi_pooling_with_relative_coordinates = roi_pooling_with_relative_coordinates(feature_map=None, rois=None, output_size=(12,12))
        self.vision_width = vision_width
        self.text_width = self.text_encoder.config.hidden_size  # i.e. cross_width 768 in bert
        self.model_map = Mapper()
        self.sat_proj = nn.Linear(1024, 768)
        if use_contrastive_loss:
            self.embed_dim = config['embed_dim']  # 256 in the config
        ...
        if use_spatial_loss:
            self.spatial_head = build_mlp(input_dim=self.text_width,
                                          output_dim=6)  # （左 中 右）  （0， 1， 2） （上 中 下）  （0， 1， 2）
    ...
    # def get_feature_maps(self, image, size):
    #     encoder =
    ...
    def predict_bbox(self, image_embeds, text_embeds, text_atts):
        ...
        # print('Here is the the output_cls', output_cls)
        # print('Here is the shape',output_cls.shape)
        # print('Here is the image_embeds',image_embeds)
        # print('Here is the image_embeds',image_embeds.shape)
        # print('Here is the text_embeds',text_embeds)
        # print('Here is the text_embeds',text_embeds.shape)
        output_coord = self.bbox_head(output_cls).sigmoid()

        return output_coord

    def predict_match_bbox(self, image_embeds, text_embeds, text_atts):
        """
        Args:
            image_embeds: encoding full images
    ...
    def bbox_feature_map(self, roi, feature_map):
        output = roi_pooling_with_relative_coordinates(feature_map, roi, (12, 12)).view(1, 144, 1024)
        # print(f'here is the output {output.shape}')
        # 我们的  size是 12 *12
