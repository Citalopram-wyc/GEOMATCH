...
from dataset.bbox_match_dataset import text2match_dataset, text2match_val_dataset,match_val_dataset
...
def create_dataset(dataset, config, evaluate=False):
    ...
    if dataset == 'pretrain':
        ...
        region_dataset = RegionTextJsonDataset(config, config['train_file_regions'],
    ...
    # 修改
        ...
        train_dataset = re_dataset_bbox(config['train_file'], train_transform, config['image_root'], mode='train',
                                        config=config)
        return train_dataset, test_dataset

    elif dataset == 'text2match':
        test_dataset = text2match_val_dataset(config['test_file'], test_transform, config['image_root'])
        if evaluate:
            return None, test_dataset
        ...
        train_dataset = text2match_dataset(config['train_file'], train_transform, config['image_root'], mode='train',
                                           config=config)
        return train_dataset, test_dataset

    elif dataset == 'match_eval':
        test_dataset = match_val_dataset(test_transform, config['val_image_root'])
        if evaluate:
            return None, test_dataset
        ...
        test_dataset = grounding_dataset_bbox(config['test_file'], test_transform, config['image_root'], mode='test',
                                              config=config)
        if evaluate:
            return None, test_dataset
        ...
        train_dataset = grounding_dataset_bbox(config['train_file'], train_transform, config['image_root'],
        ...
        train_dataset = coco_karpathy_train(train_transform, config['image_root'], config['train_file'],
