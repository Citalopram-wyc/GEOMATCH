...
class SmoothedValue(object):
    ...
    def update(self, value, n=1):
        # print(f"Before update - deque: {list(self.deque)}, count: {self.count}, total: {self.total}")
        # print(f"Updating SmoothedValue ID: {id(self)} with value: {value}, count: {n}")
        # print(f"Before update - ID: {id(self)}, deque: {list(self.deque)}, count: {self.count}, total: {self.total}")
        self.deque.append(value)
        self.count += n
        self.total += value * n
        # print(f"After update - ID: {id(self)}, deque: {list(self.deque)}, count: {self.count}, total: {self.total}")

        # print(f"After update - deque: {list(self.deque)}, count: {self.count}, total: {self.total}")

    def synchronize_between_processes(self):
        if not is_dist_avail_and_initialized():
            return
        t = torch.tensor([self.count, self.total], dtype=torch.float64, device='cuda')
        print(f"Before sync - Count: {self.count}, Total: {self.total}")
        dist.barrier()
        dist.all_reduce(t)
        t = t.tolist()
        self.count = int(t[0])
        self.total = t[1]
        print(f"After sync - Count: {self.count}, Total: {self.total}")
    ...
    def global_avg(self):
        ...
        # if self.count == 0:
        #     print("Warning: count is 0. Returning default value 0.0.")
        #     return 0.0
...
###
    def __str__(self):
        ...
        for name, meter in self.meters.items():
            if meter.count > 0:  # 仅在 count > 0 时处理
                loss_str.append(
                    "{}: {}".format(name, str(meter))
                )
            else:
                loss_str.append(
                    "{}: Not Updated".format(name)  # 或者其他信息，表示未更新
                )
    ...
    def add_meter(self, name, meter):
        ...
        #     print(f"Meter '{name}' already exists with ID {id(self.meters[name])}")
        # else:
        #     print(f"Adding meter '{name}' with ID {id(meter)}")
        self.meters[name] = meter

    def log_every(self, iterable, print_freq, header=None, dataset_len=None, epoch_info=None):
        # for name, meter in self.meters.items():
        #     print(f"Meter '{name}' - ID: {id(meter)}, deque: {list(meter.deque)}, count: {meter.count}")
        if not header:
            header = ''
        if not dataset_len:
        ...
        # print(f"iter_time ID: {id(iter_time)}")
        # print(f"data_time ID: {id(data_time)}")
...
def is_dist_avail_and_initialized():
    ...
    if not dist.is_initialized():
        return False
    return True
print(f"Distributed mode: {is_dist_avail_and_initialized()}")
...
#
# metric_logger.update(loss=0.123)
# metric_logger.update(lo=123)
# print(metric_logger)
# metric_logger.add_meter('loss', SmoothedValue())
# metric_logger.update(loss=0.456)
# metric_logger.update(lo=123)
# print(metric_logger)